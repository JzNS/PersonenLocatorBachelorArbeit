...
class ClientController(QObject):
    ...
    sig_open_calibration: pyqtSignal = pyqtSignal()
    ...
    def __init__(self, client_name: str, target_ip: str, target_port: int) -> None:
        ...
        self.camera_settings: dict[str, Any] = ConfigManager.load_camera_config()
    ...
    def worker(self) -> Optional[Any]:
        ...
        if self.calibration_ui and hasattr(self.calibration_ui, 'worker'):
            return getattr(self.calibration_ui, 'worker')
        return None
    ...
    def send_local_config_to_server(self) -> None:
        """
        Sendet NUR die spezifische Config dieses einen Clients an den Server,
        um nicht die Daten anderer Kameras zu überschreiben!
        """
        try:
            logging.info("Controller: Erkanntes Speicher-Event. Sende Config an Server...")

            camera_specific_settings: dict[str, Any] = ConfigManager.get_camera_settings(self.client_name)

            if self.sender:
                self.sender.send_db_camera_settings(self.client_name, camera_specific_settings)
                logging.info(f"Controller: Config-Update für {self.client_name} erfolgreich gesendet.")
            else:
    ...
    def check_watchdog(self) -> None:
        ...
        if self.connector and self.connector.is_connected():
            elapsed: float = time.time() - self._last_pong_time
