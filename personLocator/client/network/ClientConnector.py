...
class ClientConnector:
    ...
    def __init__(
        ...
        self.command_handler: Optional['CommandHandler'] = command_handler
    ...
    def set_command_sender(self, sender: 'CommandSender') -> None:
    ...
    def __connect_and_run(self) -> None:
        ...
        if self.command_handler and hasattr(self.command_handler, 'controller'):
            controller: Any = getattr(self.command_handler, 'controller')
            if hasattr(controller, 'on_network_connect'):
                getattr(controller, 'on_network_connect')()
    ...
    def __recv_exact(self, n: int) -> Optional[bytes]:
        ...
        while len(data) < n:
            try:
                packet: bytes = self.__socket.recv(n - len(data))
                if not packet:
                    return None
                data.extend(packet)
            except OSError:
